...
test -s skill.md
grep -q '^# Accessible Answer Key$' skill.md
grep -q 'No invented substantive content' skill.md
grep -q 'Question Source Set' skill.md
grep -q 'Answer Source Set' skill.md
...
if rg -q 'YOUR-GITHUB-USERNAME|YOUR-REPOSITORY' --glob '!tests/verify.sh' .; then exit 1; fi
